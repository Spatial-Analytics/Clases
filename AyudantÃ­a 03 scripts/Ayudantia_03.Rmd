...
Tabla_19_19 <- data.table(readRDS("Tabla_19_19.rds")) # tipo rds, no tipo excel
```
...
ggplot(data = Tabla_19_19[cae_general_red == "Ocupados",], aes(x = mes_central, y = V1, fill = sexo)) + 
  geom_bar(stat="identity") +
  scale_y_continuous(labels = number_format(scale = 0.000001, suffix = "M", big.mark = ".", decimal.mark = ",")) + 
  labs(x = "Categoría Ocupacional", y = "N° de Personas", fill = NULL) # esta malo, ya que empieza con el mes 1 pero de 2020, y luego con los otros de 2019. Además, se ve la cantidad de hombres y mujeres ocupados, pero quiero ver la variación de estos.
```
...
var_sexo[,sexo_anterior := shift(V1, n = 1, type = 'lag'), by = .(sexo)] # quiero ver las variaciones de un mes a otro. Para llevar los datos n=1 periodos al futuro, y ver la diferencia entre ese y el original.
```
...
```{r variacion}
var_sexo[,sexo_absoluto := round(V1 - sexo_anterior)] # hago la resta para ver cuánto varió de un mes a otro
var_sexo[,sexo_porcentual := round((V1 - sexo_anterior)/sexo_anterior,4)*100] # variación porcentual
```
...
var_sexo$mes_nombre <- factor(var_sexo$mes_central, levels = c(2:12,1), labels = c("FEB","MAR","ABR","MAY","JUN","JUL","AGO","SEP","OCT","NOV","DIC","ENE")) # queremos arreglar para que no aparezca primero enero de 2020 y luego febrero 2019, asi que lo ordenamos por mes.
```
...
ggplot(data = var_sexo, aes(x = mes_nombre, y = sexo_absoluto, group = sexo, color = sexo)) + 
  ...
  labs(x = "Categoría Ocupacional", y = "N° de Personas", fill = NULL) # se quitan 2 filas, ya que febrero 2019 no cuenta con datos de meses pasados
```
...
ggplot(data = var_sexo, aes(x = mes_nombre, y = sexo_porcentual, group = sexo, color = sexo)) + 
  ...
  labs(x = "Categoría Ocupacional", y = "N° de Personas", fill = NULL) # no hay diferencia entre este (variación %) y el anterior (variación absoluta)
```
...
var_sexo[, fecha := ifelse(mes_central > 9, var_sexo$mes_central, paste0("0",var_sexo$mes_central))] # quiero que todos los valores menores a 10 tengan un 0 antes (01,02,03,04,05,06,07,08,09) para que se pueda leer bien como date
...
ggplot(data = var_sexo, aes(x = fecha, y = sexo_absoluto, group = sexo, color = sexo)) + 
  geom_hline(yintercept = 0) + # pone 0 en la mitad 
  geom_line(position = position_identity()) + # agrega lineas
  geom_point(position = position_identity()) + # agrega puntos
  geom_text(aes(x = unique(var_sexo$fecha)[9], y = 0, label = "aumento\n disminución"), colour="gray60") + # agrega un texto en una ubicación en específico 
  geom_text(aes(label = sexo_absoluto), color = "black", nudge_y = 2500, size = 3) + # agrega un texto en una ubicación en específico 
  scale_y_continuous(labels = number_format(scale = 0.001, suffix = "K", big.mark = ".", decimal.mark = ",")) + # cambia formato de valores del eje y
  scale_x_date(date_labels = "%Y-%b", breaks = unique(var_sexo$fecha)[c(seq(1,12,2))]) + # cambia formato de valores del eje x
  labs(x = "Categoría Ocupacional", y = "Variación de Personas", color = NULL) # renombrar los nombres de los ejes x e y, y color = NULL lo anotamos para borrar la etiqueta "sexo"
```
...
``` {r hc line 1}
hchart(var_sexo, "line", hcaes(x = fecha, y = sexo_absoluto, group = sexo, color = sexo)) # paquete highcharter (gráfico interactivo). A diferencia de ggplot, todo se agrega en un solo comando.
```
...
var_sexo_2 <- ts(var_sexo_2[, .(Hombres,Mujeres)],start = c(2019,3), end =c(2020,1),frequency = 12) # de datatable lo pasamos a timeseries. Nos quedamos con hombres y mujeres, comenzando con el mes 3 de 2019 y terminando con enero de 2020. Frecuencia = 12 quiere decir que quiero dividir todo en 12 meses.
```
...
# Opción 2
highchart() %>%
  hc_xAxis(categories = c("Hombres", "Mujeres")) %>% # separar categorías entre hombres y mujeres
  hc_add_series(name = "Cesantes", data = col_sexo_2$Cesantes, type = "column") %>% # agregar cesantes 
  hc_add_series(name = "Inactivos", data = col_sexo_2$Inactivos, type = "column") %>% # agregar inactivos
  hc_add_series(name = "Ocupados", data = col_sexo_2$Ocupados, type = "column") %>% # agrgar ocupados
  hc_title(text = paste("Categoría laboral")) %>% # agregar título
  hc_yAxis(title = "none") # y sin titulo
```
