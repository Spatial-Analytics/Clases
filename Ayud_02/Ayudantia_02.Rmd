...
```{r cargando bases}
casos <- data.table(read_excel("COVID_19_up_to_29_02_2020_cases.xlsx"),stringsAsFactors = F) # stringsAsFactors es por si tenemos alguna variable categorizada, no queremos que se le asigne un valor numérico a esa columna
...
```{r str vs. sapply}
str(casos)
sapply(casos, FUN = class) # tengo la base de datos casos y quiero saber la clase de cada columna
...
```{r as.Date}
casos$dateRep <- as.Date(casos$dateRep,"GMT") # transformar la columna dateRep de casos a fecha
mas_dias$dateRep <- as.Date(mas_dias$dateRep,"GMT") # GMT es la zona horaria por defecto
```
...
```{r sapply}
sapply(mas_dias, FUN = unique) # trae los valores únicos de cada columnade mas_dias
```
...
cbind(casos,muertes[,.(deaths)]) # cbind pega las columnas de una base de datos al lado de las columnas de la otra. rbind hace lo mismo pero por filas.  Para que no se repitan las columnas, nos quedamos solo con la columna "deaths" de "muertes".
```
...
```{r merge}
casosymuertes <- merge(casos,muertes, by = c("day","month","year","countriesAndTerritories"), all = T,sort=F) # merge pega las filas de la base 2 que cumpla con las categorías asignadas de la base 1. Quiero unir las bases de datos de casos y muertes donde calcen todas las variables indicadas. all = T dice que se incluyan todas las filas de ambas bases de datos.
```
...
View(mas_dias)
sapply(mas_dias, FUN = class)
sapply(casosymuertes, FUN = class) #tienen las mismas variables y de la misma clase, por lo que no habría que tener problemas para unir
```

Por lo tanto podemos unir las dos bases de datos:

```{r rbind}
al_29_03_2020 <- rbind(casosymuertes,mas_dias) # se suman las filas de ambas bases de datos
```
...
al_29_03_2020[rowSums(is.na(al_29_03_2020))>0,] # me muestra las filas que contienen na
al_29_03_2020[!complete.cases(al_29_03_2020),]  ## Otro ejemplo

# Cuantas filas con NAs hay
al_29_03_2020[rowSums(is.na(al_29_03_2020))>0,.N] # me cuenta cuantas filas contienen na
...
```{r colapsar}
pormes <- chileyvecinos_completa[, .(cases = sum(cases,na.rm = T),deaths = sum(deaths,na.rm = T), popData2018 = round(mean(popData2018, na.rm = T),0)), by = c("month","countriesAndTerritories")] # colapsar es abreviar los datos
```
