...
### Repeticiones:
```{r Repeticiones} 
rep(1,4) # repetir el primer valor (1) las veces que indica el segundo valor (4)
rep("1",4)
rep(c(1,5,8,2),4)
```
...
median(c(1,9,8,4,6,3,6,5,2)) # primero te los ordena y después ve cuál está en la mitad
...
library(data.table) # data table es uno de los lenguajes más cómodos, rápidos y eficientes
library(titanic)
...
Titanic[,.(Survived,Age)]  ## También se puede con nombres de variables. Así melo tira como columnas. 
```
...
class(Titanic[,Age])
class(Titanic[,Survived]) # no existen muchas diferencias entre integer y numeric (integer se usa más que nada para menores procesamientos como datos binominales)
...
```{r N° Categorías}
length(unique(Titanic[,Age]))  # cantidad de valores diferentes que hay   
